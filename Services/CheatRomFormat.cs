...
public sealed class XplorerPro219_256Kb_Format : IRomFormat
{
...
static byte[] EncodeName(string displayName)
        ...
        var orderedGames = doc.Games
            .Where(g => !string.IsNullOrEmpty(g.Title))
            .OrderBy(g => HasStoredRawGameTitle(g) ? 1 : 0)
            .ToList();
        ...
        foreach (var g in orderedGames)
        {
            ...
            var cheats = g.Cheats
                .OrderBy(c => HasStoredRawCheatRecord(c) ? 1 : 0)
                .ToList();
            if (cheats.Count > 255)
